...
Nothing is missing any more. `image-manifest.json` lists 70 images and
`vendor-images.mjs` fetches all 70. A clean build reports `fetched 70/70` with no
warnings. If you see any failure, something has actually changed — investigate,
do not assume it is expected.

Two further images are **committed to the repo** rather than vendored, because
they were re-encoded locally and no CDN copy of the WebP exists:
`lunasea2_landscape_d63608ce.webp` and `pw_marina2_landscape_f37bf649.webp`.
Their PNG originals were 2.2 MB each; at WebP q82 they are 168 KB and 180 KB, a
~13x saving at identical dimensions. `client/public/images/` is gitignored, so
they are tracked only because they were force-added — see the note at the end.
...
| Was | Now | Why |
| --- | --- | --- |
| `IMG_0884_0eb5630e.JPG` | `lunasea2_landscape_d63608ce.webp` | Luna Sea at her actual Prince William Marina slip — transom reads `Luna Sea / OCCOQUAN, VA` |
| `IMG_0787_3b05bbdf.JPG` | `IMG_0787_03fa7333.JPG` | Live sibling; genuinely shows the 390 on the Occoquan |
| `b3_6de55c03.jpg` | `pw_marina2_landscape_f37bf649.webp` | No swim-platform photograph exists in the set, so the tile became the marina waterfront and its caption was corrected to match |
